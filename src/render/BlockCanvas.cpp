...
void BlockCanvas::flush(Window& win) {
    ...
    for (int ty = 0; ty < termH_; ++ty) {
        for (int tx = 0; tx < termW_; ++tx) {
            ...
            if (hasPrev && c.topFilled == prevCells_[idx].topFilled &&
                c.botFilled == prevCells_[idx].botFilled &&
                c.topColor == prevCells_[idx].topColor &&
                c.botColor == prevCells_[idx].botColor)
                continue;

            if (!c.topFilled && !c.botFilled) {
                if (hasPrev && (prevCells_[idx].topFilled || prevCells_[idx].botFilled))
                    win.addChar(ty, tx, ' ');
                continue;
            }
